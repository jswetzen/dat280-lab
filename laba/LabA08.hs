...
{-
This code is useful for testing the functions one by one.
...
benchTasks = do
  ...
  let task1_seq = bgroup "Sequential"
        [bench "Prelude's scanl1" (nf (scanl1 slowOp) aBitFewerInts),
         bench "Mary's recursive scanl1" (nf (sscanl1 slowOp) aBitFewerInts)]
      ...
      task2_seq = bgroup "Sequential" [bench "Given fft" (nf fft samples)]

      task2_par = bgroup "Parallel"
        -- par & pseq, some speedup
        [bench "par & pseq" (nf (pfft 5) samples),
        -- rpar & resq, makes it worse
         bench "rpar & rseq" (nf (pfft2 5) samples),
        -- Strategies (first try), really bad
         bench "Bad strategies try" (nf sfft samples),
        -- divConq with Par, the best one yet
         bench "Simon Marlow's divConq with Par" (nf spfft samples),
        -- Par monad, quite good
         bench "Par Monad" (nf (runPar . parfft 5) samples)]

      task2 = bgroup "Task 2" [task2_seq,task2_par]
  defaultMain [task1, task2] -- The rest of the tests
...
pmmkFan op (i:is) = liftM (i:) $ parMap (op i) is
...
divConq' :: NFData sol
  ...
  -> prob -> sol
divConq' indiv splt jn f prob
  = runPar $ go prob
  where
    go p
      | indiv p = return $ f p
      | otherwise  = do
        sols <- parMapM go $ splt p
        return $ jn sols
...
divConq'' f arg threshold combine divide = go arg
  where
    go a =
      case divide a of
        Nothing -> f a
        Just (l1,r1) -> combine l2 r2 `using` strat
          where
            l2      = go l1
            r2      = go r1
            strat x = do r l2; r r2; return x
                where r | threshold a = rseq
                        | otherwise = rpar
...
bflyS as = (los,rts)
  where
    ...
    los = zipWith (+) ls rs
    ros = zipWith (-) ls rs
    rts = zipWith (*) ros [tw (length as) i | i <- [0..l-1]]
